...
"KRISHA CHOUDHARY","LAKSHIKA GEHLOT","LEHAR THADHANI",
...
for(let i=0;i<40;i++){
    ...
    d.onclick=function(){
        ...
        if(d.classList.contains("green")){
            ...
            if(student){student.classList.remove("green");student.classList.add("red");}
...
function buildSidebar(){
    let c=document.getElementById("studentList");
    c.innerHTML="";
...
/* BIAS */
function setBias(){
    let input=document.getElementById("biasInput").value;
    let list=input.split(",").map(x=>parseInt(x.trim())).filter(x=>!isNaN(x));
    list=[...new Set(list)].filter(x=>x>=1&&x<=40);

    biasList=list.filter(x=>x!==4);
    alert("Bias set: "+list.join(", "));
    document.getElementById("removeBiasBtn").style.display="inline-block";
}

function removeBias(){
    biasList=[];
    document.getElementById("removeBiasBtn").style.display="none";
}
...
function randomize(){
    ...
    pool=pool.filter(x=>x!==4);
    pool=pool.filter(x=>!biasList.includes(x));
...
function resetAll(){
    ...
    document.getElementById("biasInput").value="";
    document.getElementById("removeBiasBtn").style.display="none";
}
